...
impl CoordinatorTask {
    ...
    pub fn mutate(&mut self, body: serde_json::Value) -> anyhow::Result<()> {
        self.doc.mutate_direct(|tx| -> anyhow::Result<()> {
            let id = body["id"].as_str().ok_or_else(|| anyhow!("Missing id"))?;
            let email = body["email"].as_str().ok_or_else(|| anyhow!("Missing email"))?;
            let name = body["name"].as_str().ok_or_else(|| anyhow!("Missing name"))?;
            ...
            match tx.execute("
                ...
                Ok(_) => (),
                Err(e) => console_error!("Error creating table: {:?}", e),
            }

            match tx.execute(
                "insert into users (id, email, name, created_at) values (?1, ?2, ?3, datetime('now'))",
                &[id, email, name],
            ) {
                Ok(_) => (),
                Err(e) => console_error!("Error inserting user: {:?}", e),
            }
